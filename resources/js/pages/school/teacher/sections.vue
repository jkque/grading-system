    ...
    <div class="animated fadeIn">
        <b-card no-header  v-if="isMain">
            ...
            <b-table striped hover :items="list" :fields="fields" :responsive="true" :sort-by.sync="sortBy">
                ...
                <template slot="lessonPlan" slot-scope="row">
            ...
            <template slot="header">{{activeClass ? activeClass.subject.grade_level.name : ''}} / {{activeClass ? activeClass.section.name : ''}} / {{activeClass ? activeClass.subject.name : ''}} / Student Performance / {{activePerformance.name}} / {{selectedGP.name}}</template> 
                ...
                <b-card-body class="add-section"> 
                    <div class="header">
                        ...
                        <div class="score" v-for="(field, index) in fieldsUserPerformance" :key="index">{{ activePerformance.name}} {{ field.key }} - {{getTotalScore(field.id)}} Points</div>
                    </div>
                        ...
                        <li v-for="(student, index) in filteredStudent" :key="index">
                            <div class="input-list">
                                ...
                                <div class="score" v-for="(performance, index) in student.performances" :key="index">
                                    <input v-model="performance.score" class="form-control" type="number" placeholder="Score" :max="getTotalScore(performance.performance_score_id)" min="0"> 
    ...
    <b-modal id="modalInfo" :title="modalInfo.title" v-model="modalInfoShow">
        <form @submit.prevent="updateLessonPlan" @keydown="form.onKeydown($event)">
            ...
            <div class="form-group row">
                <label class="col-md-3 col-form-label text-md-right">Lesson Plan</label>
                <div class="col-md-7">
                    <b-form-select
                        :options="planSelect"
...
export default {
    ...
    methods:{
        info (item, index, button) {
            this.form.clear();
            this.modalInfo.title = 'Add Lesson Plan';
